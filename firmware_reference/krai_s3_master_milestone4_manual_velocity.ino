...
// =====================================================
// KRAI NUC USB CDC LINK CONFIG - MILESTONE 4 MANUAL VELOCITY
// =====================================================
// IMPORTANT:
...
// tested safely without motors or sensors. Milestone 4 also accepts manual velocity commands after the ROS2 bridge enables manual control. Primitive autonomous commands are still disabled.
...
uint8_t getPrimitiveReportedState() {
  if (masterAutoMode == MASTER_AUTO_MOVE || masterAutoMode == MASTER_AUTO_TURN) {
    return PRIM_STATE_RUNNING;
  }
  if (masterAutoMode == MASTER_AUTO_DONE) {
    return PRIM_STATE_DONE;
  }
  return PRIM_STATE_IDLE;
}
...
void sendNucPrimitiveStatus() {
  ...
  if (masterAutoMode == MASTER_AUTO_MOVE) {
    p.primitive_type = 1;  // move_rel placeholder
  } else if (masterAutoMode == MASTER_AUTO_TURN) {
    p.primitive_type = 2;  // turn_rel placeholder
  } else {
    p.primitive_type = 0;
  }

  p.primitive_state = getPrimitiveReportedState();
  p.progress_x10 = floatToInt32x10Nuc(autoDebugTraveledCm);
  p.remaining_x10 = floatToInt32x10Nuc(autoDebugRemainingCm);
  p.heading_error_deg_x10 = floatToInt16x10Nuc(autoDebugHeadingErrDeg);
  p.fault_flags = getBasicFaultFlags();
...
void updateNucTelemetryTx() {
  ...
  // 5 Hz primitive status placeholder for milestone 1
  if (nowMs - lastPrimTxMs >= 200) {
    lastPrimTxMs = nowMs;
...
void updateFakeDataMode() {
  ...
  if (manualControlActive) {
    ...
    // Simulated heading slowly moves around zero so ROS2 odometry visibly changes.
    float rawHeadingDeg = 25.0f * sinf(t * 0.35f);
    headingDeg = normalize180(rawHeadingDeg - fakeHeadingZeroRawDeg);
    yawRawDeg = normalize360(rawHeadingDeg);
    poseThetaDeg = headingDeg;

    // Simulated forward velocity in cm/s.
    poseLinearCmS = 12.0f + 4.0f * sinf(t * 0.70f);
  }
  headingReady = true;
  ...
  // Simulated primitive progress for the GUI/status panel.
  fakePrimitiveProgressCm += poseLinearCmS * dtSec;
  if (fakePrimitiveProgressCm >= 100.0f) {
    fakePrimitiveProgressCm = 0.0f;
  }
  autoDebugTraveledCm = fakePrimitiveProgressCm;
  autoDebugRemainingCm = 100.0f - fakePrimitiveProgressCm;
  autoDebugHeadingErrDeg = 3.0f * sinf(t * 0.90f);
#endif
}
